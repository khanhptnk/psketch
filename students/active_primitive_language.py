...
class ActivePrimitiveLanguageStudent(InteractivePrimitiveLanguageStudent):
    ...
    def __init__(self, config):
        ...
        super(ActivePrimitiveLanguageStudent, self).__init__(config)
        self.n_actions = config.student.model.n_actions
    ...
    def set_instructions(self, instructions, is_eval):
        ...
        self.instructed_model.init(self.batch_size, instruction_encodings,
            src_mask=instruction_mask)
    ...
    def receive(self, descriptions, ask_actions):
    ...
    def instructed_act(self, states, ask_actions):
        state_features = [state.features() for state in states]
        state_features = self._to_tensor(state_features).float()
        ...
        for i in range(self.batch_size):
            if ask_actions[i] == self.ASK:
                self.global_action_seqs[-1][i] = actions[i]
